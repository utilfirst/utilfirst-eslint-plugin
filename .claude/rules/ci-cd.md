...
- Pin every third-party GitHub Actions `uses:` to a full-length commit SHA, with a trailing `# vX.Y.Z` comment for readability and Dependabot updates (a tag can be repointed at malicious code while a SHA cannot; first-party GitHub-maintained actions like `actions/checkout` may pin by tag under their verified-publisher trust model)
