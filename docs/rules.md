...
- `no-known-value-widening` rejects known values assigned, returned, or asserted into broad target types such as `any`, `unknown`, and `object`, including aliases that resolve to them. Keep inference, use a named owner type, or use `satisfies` when a value needs contract checking without changing its inferred type. Explicit dictionary and generic-container annotations remain valid accumulator contracts for object literals.
...
- `no-call-count-only-test` rejects test cases whose assertions only inspect mock call counts or omission. Vitest and Jest expectations plus Node assert calls count as assertion evidence. Assert a returned value, rendered output, emitted payload, persisted state, or stateful fake outcome. Payload assertions such as `toHaveBeenCalledWith(...)` remain valid. Use a reasoned suppression when multiplicity or omission is itself the boundary contract, such as retry limits or at-most-once delivery.
- `no-imported-constant-restatement` rejects equality assertions between an imported uppercase constant or its static member and a static literal. It recognizes Vitest and Jest `expect(...)` equality matchers plus Node assert equality methods, including strict assertion objects. Exercise behavior that consumes the constant. Use a reasoned suppression when the exact literal is an external protocol or public contract.
- `no-module-mocking` rejects Vitest and Jest mocks of repository-owned modules, including namespace controller calls and Vitest's typed `vi.mock(import(...))` form. Inject an unowned dependency through a production boundary and test with a real implementation or stateful fake. `internalModulePrefixes` marks package-style workspace imports as repository-owned.
- `no-negated-throw-assertion` rejects `expect(fn).not.toThrow()`, Node `assert.doesNotThrow(...)`, Node `assert.doesNotReject(...)`, and their supported aliases. Invoke or await the operation directly so a failure rejects the test, then assert any returned value or observable state that carries the behavior contract.
...
- `no-uncontrolled-time-in-test` rejects global `Date()`, `Date.now()`, and zero-argument `new Date()` reads in test setup, hooks, and test cases unless the owning execution scope sets system time or configures fake timers with a `now` value. A suite hook controls tests in that suite but a controller call in one test does not exempt another test. Inject time or control the clock before reading it so each test has deterministic inputs.
- `require-repository-test-subject` rejects Vitest, Jest, and Node test files that do not import repository-owned runtime code. Relative, absolute, `#`, `@/`, `~/`, and Cloudflare runtime specifiers count by default. `internalModulePrefixes` extends repository ownership to package-style imports. Runtime static and dynamic imports establish the subject; type-only imports do not. Remove tests of library behavior or import the repository behavior whose failure the case is meant to identify.
