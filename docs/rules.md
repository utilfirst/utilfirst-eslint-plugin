...
Every exported rule is enabled at error severity by `configs.recommended`. The config also enables ESLint's `no-else-return` and `no-nested-ternary` rules. Options adapt repository ownership or externally fixed signatures while leaving the rule enabled.
