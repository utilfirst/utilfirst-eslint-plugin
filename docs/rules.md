...
- `no-call-count-only-test` rejects test cases whose assertions only inspect mock call counts or omission. Assert a returned value, rendered output, emitted payload, persisted state, or stateful fake outcome. Payload assertions such as `toHaveBeenCalledWith(...)` remain valid. Use a reasoned suppression when multiplicity or omission is itself the boundary contract, such as retry limits or at-most-once delivery.
...
- `no-negated-throw-assertion` rejects `expect(fn).not.toThrow()` and its Vitest and Jest aliases. Invoke the function directly so an exception fails the test, then assert any returned value or observable state that carries the behavior contract.
...
- `no-test-snapshots` rejects Vitest and Jest snapshot matchers and snapshot serializers. Assert the exact observable fields that carry the contract. Use a reasoned suppression when a reviewed serialized public artifact must remain byte-stable.
- `no-truthy-falsy-assertion` rejects `toBeTruthy()` and `toBeFalsy()`, including negated forms. Assert the exact expected value so the test preserves value and type information.
...
- `require-repository-test-subject` rejects files with `test(...)` or `it(...)` cases that do not import repository-owned code through a relative, absolute, `#`, `@/`, `~/`, or Cloudflare runtime specifier. Static and dynamic imports both establish the subject. Remove tests of library behavior or import the repository behavior whose failure the case is meant to identify.
