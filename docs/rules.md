...
- `no-imported-constant-restatement` rejects direct equality assertions between an imported uppercase constant and a static literal. Exercise behavior that consumes the constant. Use a reasoned suppression when the exact literal is an external protocol or public contract.
